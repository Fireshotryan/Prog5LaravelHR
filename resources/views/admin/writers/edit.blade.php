...
@endif
    ...
    action="{{ route('writers.update',$writers->id) }}" method="POST">
    @method('PUT')
    @csrf
        ...
        <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tigh focus:outline-none focus:shadow-outline @error('name') border-red-500 @enderror" name="name" id="name" value="{{$writers->name}}" type="text">
    </div>
